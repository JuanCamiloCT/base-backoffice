...
function isEditable(node) {
    return (node && $$1(node).hasClass('note-editable')) || (node && $$1(node).attr('contenteditable'));
}
/**
 ...
 * @return {Node} - new node
 */
function isSaltoPagina(node) {
    return $$1(node).hasClass('saltopagina') || $$1(node).hasClass('remove');
}
...
var Style = /** @class */ (function () {
    ...
    Style.prototype.fromNode = function ($node) {
      ...
      // se toma el valor en pixeles y se transforma en puntos para que sea compatible con archivos Word
       if (styleInfo['font-size'].indexOf("px") !== -1) {
        styleInfo['font-size'] = Math.ceil(3 / 4 * parseInt(styleInfo['font-size'])) + "pt";
      }
      return styleInfo;
    };
    /**
...
var Editor = /** @class */ (function () {
    function Editor(context) {
      ...
      this.context.memo('help.cortar', this.lang.help.cortar);
      this.context.memo('help.copiar', this.lang.help.copiar);
      this.context.memo('help.pegar', this.lang.help.pegar);
      this.context.memo('help.buscar', this.lang.help.buscar);
      this.context.memo('help.seleccionar', this.lang.help.seleccionar);
      ...
      for (var idx = 1; idx <= 6; idx++) {
        ...
        * @param {Node} node
        */
       this.insertChild = this.wrapCommand(function (node) {
           if (_this.isLimited($$1(node).text().length)) {
               return;
           }
           var rng = _this.createRange();
           rng.sc.parentNode.appendChild(node);
       });

       /**
        * insertar salto de pagina con un espacio antes y despues del bloque
        * @param {String} markup
        */
       this.insertarSaltoPagina = this.wrapCommand(function () {
            var ht = document.createElement('div');
            ht.className = 'saltopagina';
            ht.setAttribute('contenteditable', 'false');
            ht.innerHTML = '<div class="remove" style="margin-top: 0px;padding: 2px; color: #ccc;width: 100%;display: block;">'
                .concat('<div style="width: 40%;display: inline-block;"></div>')
                .concat('<span class="remove" style="text-align:center; font-style: italic;color: #ccc; width: 20%;">')
                .concat('Salto de página')
                .concat('</span>')
                .concat('<div style="width: 40%;display: inline-block;"></div>')
                .concat('</div>');
            ...
            if (rng.sc.nodeName == '#text') {
                rng.sc.parentNode.appendChild(ht);
                rng.sc.parentNode.appendChild(document.createElement('br'));
            } else {
                rng.sc.appendChild(ht);
                rng.sc.appendChild(document.createElement('br'));
            }            
       });
       /**
         * eliminar salto de pagina
         */
       this.eliminarSaltoPagina = this.wrapCommand(function () {
            var $target = $$1(_this.context.targetSaltoPagina).parent().parent();
            $target.remove();
        });
        /**
         * insert text
    ...
    Editor.prototype.insertImage = function (src, param) {
        ...
        return createImage(src, param).then(function ($image) {
            ...
            if (typeof param === 'function') {
                ...
                // se agrega esta clase para que no se agregue al final del tag
                $image.addClass('imagen');
                if (typeof param === 'string') {
                    $image.attr('data-filename', param);
                    // este atributo del tag permite detectar el final del tag, no borrar
                    $image.attr('data-end', "end");
                }
                // este es el tamaño maximo ancho de tamaño carta 21.6cm menos 6.8cm
                var maxW = 816 - 257; // 559 pixeles ancho maximo de una imagen
                $image.css('width', Math.min(maxW, $image.width()));
                $image.css('max-width', maxW);
            }
            $image.show();
...
var Clipboard = /** @class */ (function () {
    ...
    function onButtonClicked() {
        var files = selectFile("image/*", true).then(function(res) {
            _this.context.invoke('editor.insertImagesOrCallback', res);
        });
    ...
    function obtenerURL(str) {
        $$1(str).find('img').each(async function(index, value) {
            imgURL.push($$1(this)[0].src.split('file:///')[1].replace(new RegExp('/', 'g'),  '\\'));
        });
    }
        ...
        if (clipboardData && clipboardData.items && clipboardData.items.length) {
            ...
            for (var i = 0; i < clipboardData.items.length; i++) {
                if ( clipboardData.items[i].type == 'text/html') {
                    clipboardData.items[i].getAsString(obtenerURL);
                }
            }
...
var Handle = /** @class */ (function () {
    ...
    Handle.prototype.update = function (target) {
        ...
        this.context.invoke('saltoPaginaPopover.update', target);
        if (isImage) {
            var $image = $$1(target);
...
var Buttons = /** @class */ (function () {
    ...
    Buttons.prototype.initialize = function () {
        ...
        this.addSaltoPaginaPopoverButtons();
    ...
    /**
     * saltoPagina : [
     *   ['remove', ['eliminarSaltoPagina']]
     * ],
     */
    Buttons.prototype.addSaltoPaginaPopoverButtons = function () {
        var _this = this;
        // Remove Buttons
        this.context.memo('button.eliminarSaltoPagina', function () {
            return _this.button({
                contents: _this.ui.icon(_this.options.icons.trash),
                tooltip: _this.lang.saltoPagina.remove,
                click: _this.context.createInvokeHandler('editor.eliminarSaltoPagina')
            }).render();
        });
    };
...
var Toolbar = /** @class */ (function () {
    ...
    Toolbar.prototype.followScroll = function () {
        ...
        var editorWidth = '100%'; // this.$editor.width();
    ...
    ImageDialog.prototype.initialize = function () {
        ...
        if (this.options.maximumImageFileSize) {
            ...
            var readableSize = (this.options.maximumImageFileSize / Math.pow(1024, unit)).toFixed(2) * 1 +
                ' ' + ' KMGTP'[unit] + 'B';
            imageLimitation = "<span>" + (this.lang.image.maximumFileSize + ' : ' + readableSize) + "</span>";
        }
        var body = [
            '<div class="form-group note-form-group note-group-select-from-files">',
            '<label class="note-form-label">' + this.lang.image.selectFromFiles + '</label>',
            '<input class="note-image-input note-form-control note-input" ',
            ' type="file" name="files" accept=".png, .jpg, .jpeg" multiple="multiple" />',
            imageLimitation,
            '<br>',
            '<span>Formatos aceptados: PNG, JPG, JGPE</span>',
            '</div>',
...
/**
 * Salto de linea popover module
 *  mouse events that show/hide popover will be handled by Handle.js.
 *  Handle.js will receive the events and invoke 'saltoPaginaPopover.update'.
 */
var SaltoPaginaPopover = /** @class */ (function () {
    function SaltoPaginaPopover(context) {
        var _this = this;
        this.context = context;
        this.ui = $$1.summernote.ui;
        this.editable = context.layoutInfo.editable[0];
    ...
    SaltoPaginaPopover.prototype.shouldInitialize = function () {
        return !lists.isEmpty(this.options.popover.saltoPagina);
    };
    SaltoPaginaPopover.prototype.initialize = function () {
        try {
            this.$popover = this.ui.popover({
                className: 'note-salto-pagina-popover'
            }).render().appendTo(this.options.container);
            var $content = this.$popover.find('.popover-content,.note-popover-content');
            this.context.invoke('buttons.build', $content, this.options.popover.saltoPagina);
        } catch(e) {
        }
    };
    SaltoPaginaPopover.prototype.destroy = function () {
        this.$popover.remove();
    };
    SaltoPaginaPopover.prototype.update = function (target) {
        if (dom.isSaltoPagina(target)) {
            this.context.targetSaltoPagina = target;
            var pos = dom.posFromPlaceholder(target);
            var posEditor = dom.posFromPlaceholder(this.editable);
            this.$popover.css({
                display: 'block',
    ...
    SaltoPaginaPopover.prototype.hide = function () {
        this.$popover.hide();
    };
    return SaltoPaginaPopover;
}());
...
var HelpDialog = /** @class */ (function () {
    ...
    HelpDialog.prototype.createShortcutList = function () {
        ...
        return Object.keys(newKeyMap).map(function (key) {
            var command = newKeyMap[key];
...
$$1.summernote = $$1.extend($$1.summernote, {
    ...
    options: {
        modules: {
            ...
            'saltoPaginaPopover': SaltoPaginaPopover,
